...
export async function POST(req: NextRequest) {
  try {
    ...
    const priceId = requireEnv("STRIPE_PRICE_CAD_19");
    ...
    const followerRange = payload.followerRange as string | undefined;

    if (!industry || !followerRange || !getIndustry(industry) || !getFollowerRange(followerRange)) {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      metadata: { orderId, email, industry, followerRange },
